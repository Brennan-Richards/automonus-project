...
class PopulateIncomeTest(TestCase):
    ...
    def test_base(self):
        institution = test_model.Institution.objects.create(name="Wells Fargo TEST", plaid_id="FIRST")
        ...
        income = Income.objects.get(user_institution=self.first_user_institution)
        income_stream = IncomeStream.objects.filter(income=income)

        first_inc_resp = INCOME_RESPONSE[ACCESS_TOKEN_1]['income']

        #  check fields value
        self.assertEqual(first_inc_resp['max_number_of_overlapping_income_streams'], income.max_number_of_overlapping_income_streams)
        self.assertEqual(first_inc_resp['number_of_income_streams'], income.number_of_income_streams)

        self.assertEqual(first_inc_resp['last_year_income_before_tax'], income.last_year_income_before_tax)
        self.assertEqual(first_inc_resp['last_year_income'], income.last_year_income_minus_tax)


        self.assertEqual(first_inc_resp['projected_yearly_income_before_tax'], income.projected_yearly_income_before_tax)
        self.assertEqual(first_inc_resp['projected_yearly_income'], income.projected_yearly_minus_tax)

        # check calculate
        self.assertEqual(income.last_year_taxes, round(income.last_year_income_before_tax - income.last_year_income_minus_tax, 2))
        self.assertEqual(income.projected_yearly_taxes, round(income.projected_yearly_income_before_tax - income.projected_yearly_minus_tax, 2))

        # check count of array
        self.assertEqual(income_stream.count(), 1, "check count income_streams")

    def test_empty_stream(self):
        institution = test_model.Institution.objects.create(name="Wells Fargo TEST", plaid_id=EMPTY_STREAMS)
        ...
        # check count of array
        self.assertEqual(income_stream.count(), 0, "check count income_streams")
